...
def test_neutraliseLaTeXComments():
    body = 'some thing\n'
...
def test_findMacros():
    ...
    # Make sure that findMacro does not pick up escaped special LaTeX symbols.
    body = r'\# \$ \% \&  \^ \_ \{ \} \~'
    _, out = findMacros(body)
    assert len(out) == 0
...
def test_findDelimiters_escaped():
    body = r'a {b\$ c \{}'
    out = findDelimiters(body)
    assert len(out) == 6
    assert out[0] == ((0, 0), True,  'text', None)
...
def test_pruneDelimiters():
    ...
    body = r'foo \begin{bar}\begin{foo}\begin{bar}\end{bar}\end{foo}\end{bar}'
    out = findDelimiters(body)
    out = pruneDelimiters(out, plugins=['bar'])
    assert len(out) == 6


def test_buildTree_text():
    body = 'a\n'
...
def test_buildTree_macroonly_multi():
    body = r'\ldots \vdots '
    delim_list = findDelimiters(body)
    delim_list = pruneDelimiters(delim_list)
    out = buildTree(body, delim_list)
...
def test_buildTree_beginendonly():
    body = r'\begin{foo}bar\end{foo}'
    delim_list = findDelimiters(body)
    delim_list = pruneDelimiters(delim_list)
    out = buildTree(body, delim_list)
...
def test_buildTree_mixed_nested():
    body = '{a0 {$a1$}}'
    delim_list = findDelimiters(body)
    delim_list = pruneDelimiters(delim_list)
    out = buildTree(body, delim_list)
...
def test_convertTreeToHTML_macro_noplugin():
    body = r'\fbox{a}b '
    delim_list = findDelimiters(body)
    delim_list = pruneDelimiters(delim_list)
...
def test_convertTreeToHTML_macro_noplugin_rectbracket():
    body = r'\ldots[a]b '
    delim_list = findDelimiters(body)
    delim_list = pruneDelimiters(delim_list)
    root = buildTree(body, delim_list)
...
def test_convertTreeToHTML_plugin_env_bug1():
    """
...
if __name__ == '__main__':
    ...
    test_findNewline()
    test_findMacros()
    test_findMacros_single()
    test_findMacros_multi()
    test_findDelimiters_text()
    test_findDelimiters_escaped()
    test_findDelimiters_mixed()
    test_pruneDelimiters()
    test_buildTree_text()
    test_convertTreeToHTML_macro_noplugin_rectbracket()
    test_convertTreeToHTML_macro_noplugin()
    test_convertTreeToHTML_plugin_macro()
    test_convertTreeToHTML_plugin_env()
    test_convertTreeToHTML_1()
    test_buildTree_beginendonly()
    test_buildTree_mixed_flat()
    test_buildTree_mixed_nested()
    test_buildTree_braceonly()
    test_buildTree_dollaronly()
    test_buildTree_macroonly_multi()
    test_sanitisePreamble()
    test_neutraliseLaTeXComments()
    test_splitLaTeXDocument()
