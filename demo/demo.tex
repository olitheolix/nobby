...
\begin{theorem}[Gauss Law]
  Let $\mathcal{V}$ be a volume and $\mathcal{S}$ its surface. Then
  \begin{align}
    \iiint_{\mathcal{V}} 1\; \textnormal{d}\mvec{v}
    = \oint_{\mathcal{S}} 1\; \textnormal{d}\vec{s}.
  \end{align}
\end{theorem}
...
the \LaTeX counters right before the environments listed in
\texttt{config.counter\_dump\_envs}. Add ``theorem'' to that list and
run Nobby again to rectify the problem.
